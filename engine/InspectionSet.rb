...
class InspectionSet
  ...
  def field
    ...
    loop do
      ...
      r O4.tt.error.inspection.element.add.field.max_length if length > O4.pp.element.add.field.max_length
      r O4.tt.error.inspection.element.add.field.min_length if length < O4.pp.element.add.field.min_length
      fields[field] = @env.json[field]
    end
    r O4.tt.error.inspection.element.add.field.min_count if count < O4.pp.element.add.field.min_count
    r O4.tt.error.inspection.element.add.field.max_count if count > O4.pp.element.add.field.max_count
    return fields
  end
end